...
NAME = 'humanity'
ENABLE = True
PATTERN = r'^!cahb? (?P<phrase>.*)'
USAGE = '''Usage: !cah[b] <phrase>
Plays bots against humanity 

Example:

    > !cah foobar
    What's my super power? foobar

    >!cahb I think that _ is _
    I think that Santa Clause is Dead parents
''' 
...
async def humanity(bot, message, phrase):

    # determine white or black (not jank)
    if 'b' in str(message).split(' ')[2]:
        card_text = phrase.strip()
        # replace every instance of _ in card_text with something from WHITES
        replaced = sub(r'\b_\b', blankfiller, card_text)
