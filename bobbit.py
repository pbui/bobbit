...
class Bobbit(object):
    ...
    def handle_channel_message(self, nick, channel, message):
        ...
        for response in self.process_message(nick, message, channel):
    ...
    def handle_private_message(self, nick, message):
        ...
        for response in self.process_message(nick, message):
            self.send_response(response, nick=nick)

    # Modules ------------------------------------------------------------------

    def process_message(self, nick, message, channel=None):
        for pattern, callback in self.modules:
            match = pattern.match(message)
            if match:
                yield callback(nick, message, channel, **match.groupdict())
